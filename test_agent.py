    ...
    try:
        ...
        if result_state.get("database_connection"):
            ...
            if schema_state.get("schema_info"):
                ...
                print(f"Schema info length: {len(schema_state['schema_info'])} characters")
                
                # Show a sample of the schema
                schema_sample = schema_state['schema_info'][:500] + "..." if len(schema_state['schema_info']) > 500 else schema_state['schema_info']
                ...
                print(f"✓ Database contains {len(tables)} tables (showing first 5): {[t[0] for t in tables]}")
